...
OutFile "wc-ng-v${VERSION}_windows_setup.exe"
...
Section "Sauerbraten WC NG (required)"
    ...
    WCConfigFound:
        Rename "$INSTDIR\wc_backup.cfg" "$INSTDIR\wc.cfg"
    WCConfigNotFound:
...
Section "Uninstall"
    ...
    IfFileExists "$INSTDIR\wc.cfg" WCConfigFound WCConfigNotFound
    WCConfigFound:
        Delete "$INSTDIR\wc_backup.cfg"
        Rename "$INSTDIR\wc.cfg" "$INSTDIR\wc_backup.cfg"
    WCConfigNotFound:
