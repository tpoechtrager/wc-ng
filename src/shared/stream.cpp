...
char *makerelpath(const char *dir, const char *file, const char *prefix, const char *cmd)
{
    ...
    if(prefix) copystring(tmp, prefix, sizeof(localtmp)); //NEW sizeof(localtmp)
    else tmp[0] = '\0';
    if(file[0]=='<')
    {
    ...
    if(cmd) concatstring(tmp, cmd, sizeof(localtmp)); //NEW sizeof(localtmp)
    if(dir)
    {
        defformatstring(pname, "%s/%s", dir, file);
        concatstring(tmp, pname, sizeof(localtmp)); //NEW sizeof(localtmp)
    }
    else concatstring(tmp, file, sizeof(localtmp)); // sizeof(localtmp)
    return tmp;
}
...
char *path(const char *s, bool copy)
{
    static string localtmp; //NEW tmp -> localtmp
    char *tmp = (char*)mod::allocatethreadstorage(0, sizeof(localtmp), true, localtmp); //NEW
    copystring(tmp, s, sizeof(localtmp)); //NEW sizeof(localtmp)
    path(tmp);
    return tmp;
}
...
const char *findfile(const char *filename, const char *mode)
{
    ...
    if(homedir[0])
    {
        nformatstring(s, sizeof(locals), "%s%s", homedir, filename); // formatstring -> nformatstring
    ...
    loopv(packagedirs)
    {
        ...
        nformatstring(s, sizeof(locals), "%s%s", pf.dir, filename); // formatstring -> nformatstring
        if(fileexists(s, mode)) return s;
    }
