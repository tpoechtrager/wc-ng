...
//NEW
// glGetString(GL_RENDERER) doesn't work with multipoll
static mod::strtool sgfxvendor;
static sdlmutex getgfxvendormutex;
mod::strtool getgfxvendor()
{
    SDL_Mutex_Locker ml(getgfxvendormutex);
    mod::strtool buf;
    copystrtool(buf, sgfxvendor);
    return buf;
}
//NEW END
    ...
    if(!hidehud && !mainmenu)
    {
        if(!hidestats)
        {
            ...
            if(showfps)
            {
                ...
                draw_textf("fps %d"/*" draw %d.%dms ifps %d"*/, conw-5*FONTH, conh-FONTH*3/2, getfps(0)/*, decimilli/10, decimilli%10, getfps(1)*/);
                roffset += FONTH;
            }
...
static drawerjob job = DRAWER_NONE;
static SDL_mutex *screenmutex = NULL;
static bool keepgl_;
static SDL_atomic_t swapping_;
static SDL_sem *dojob, *donejob;
static SDL_Thread *drawerthread = NULL;
static volatile SDL_threadID drawerid;
...
void initializedrawer()
{
    ...
    SDL_AtomicSet(&swapping_, 0);
    keepgl_ = multipoll == 1;
    dojob = SDL_CreateSemaphore(0);
    donejob = SDL_CreateSemaphore(0);
    drawerthread = SDL_CreateThread(drawerfn, "drawer", NULL);
    drawerid = SDL_GetThreadID(drawerthread);
}

static bool isdrawerthread()
{
...
{
    return SDL_AtomicGet(&swapping_)!=0;
}

static void dispatchjob(drawerjob j)
{
    job = j;
    SDL_SemPost(dojob);
    SDL_SemWait(donejob);
...
void drawer::draw()
{
    ...
    if(isdrawerthread()) SDL_AtomicSet(&swapping_, 0);
}
...
drawer::drawer()
{
    ...
    if(recursion++) return;
    if(keepgl_ && !isdrawerthread()) dispatchjob(DRAWER_RELEASE);
    extern SDL_GLContext glcontext;
...
drawer::~drawer()
{
    if(!--recursion && keepgl_ && !isdrawerthread())
    {
        SDL_GL_MakeCurrent(NULL, NULL);
        dispatchjob(DRAWER_ACQUIRE);
    }
