...
#error remove ICOMMAND(kill, ...);
    ...
    static int compareplayer(const mod::extinfo::playerinfo *a, const mod::extinfo::playerinfo *b)
    {
    ...
    void renderextinfo(g3d_gui& g, mod::strtool& command, const ENetAddress& eaddr, bool waiting,
    ...
    {
        string errbuf;
        mod::strtool errstr(errbuf, sizeof(errbuf));
        ...
        mod::extinfo::extteaminfo *extteaminfo = (mod::extinfo::extteaminfo*)ti;
        vector<mod::extinfo::playerinfo>& players = *(vector<mod::extinfo::playerinfo>*)pplayers;
        ...
        vector<mod::extinfo::playerinfo*> specs;
        ...
        loopv(teams)
        {
            ...
            {
                bool ignored = mod::ipignore::isignored(ep.ip.ui32);
                if(ignored)
                {
                    g.pushlist();
                    g.background(ipignorecolor, 3);
                }
                g.text(ep.getname(), gamemod::guiprivcolor(ep.priv), NULL);
                if(ignored) g.poplist();
            });
            g.poplist();

            extern int showcountry;
            if (showcountry && gamemod::haveextinfoplayerips(gamemod::EXTINFO_IPS_SERVERBROWSER, &players))
            {
                g.pushlist();
                g.text("country", color);
                g.strut(8);
                loopexintofplayerinfos(epi, game::rendercountry(g, epi.countrycode, epi.country ? epi.country : "??", showcountry));
                g.poplist();
            }
        ...
        if (haspass)
        {
            ...
            {
                connect:;
                mod::strtool addrbuf(32), passbuf(haspass ? 32 : 0);
                mod::escapecubescriptstring(addr, addrbuf);
                if (haspass) mod::escapecubescriptstring(pass, passbuf);
        ...
        {
            if (haspass || *mod::bouncerhost) g.separator();
    ...
    const char *wcconfig() { return "wc-ng-sdl2.cfg"; } //NEW
